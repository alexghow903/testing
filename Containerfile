...
RUN --mount=type=cache,dst=/var/cache/rpm-ostree \
    ...
    sudo curl --output-dir "/etc/yum.repos.d/" --remote-name https://copr.fedorainfracloud.org/coprs/g/exotic-soc/bc250-mesa/repo/fedora-"${FEDORA_MAJOR_VERSION}"/group_exotic-soc-bc250-mesa-fedora-"${FEDORA_MAJOR_VERSION}".repo && \
...
RUN echo "Installing GPU governor... " && \
    rpm-ostree install --cache-only /tmp/oberon-governor-1.0.0-1.x86_64.rpm && \
